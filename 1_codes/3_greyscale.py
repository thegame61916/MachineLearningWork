...
for xe in range(len(X_train)): 
	x_train1 = X_train[xe]
	x_grey = x_train1
	for j in range(r):
		for k in range(c):
			x_grey[0][j][k] =  ( (0.3 * x_train1[0][j][k]) + (0.59 * x_train1[1][j][k]) + (0.11 * x_train1[2][j][k]) )
			x_grey[1][j][k] = 0
			x_grey[2][j][k] = 0
	np.append(X_Grey, x_grey)
	np.append(Y_train, Y_train[xe])
np.append(X_train, X_Grey)
